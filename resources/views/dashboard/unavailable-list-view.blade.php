...
@section('page-css')
    ...
    <style>
        ...
        /*  select.filter{
              background: url({{url('/')}}/public/images/angle-arrow-down-black.png) no-repeat 22rem transparent !important;
        }*/
        /* select#category{
...
@section('main-content')
    ...
    <ul class="nav nav-tabs" id="myTab" role="tablist">
        ...
        <li class="nav-item">
            <a class="nav-link active" href="{{route('items-availability')}}">Item(s) Availability</a>
        </li>
        <li class="nav-item">
    ...
    <div class="row p-3" id="top-area">
        ...
        @if($availability  &&  $items  && $category  && $menu &&  $all_groups  && $menu_type &&  $restaurant  && $another_restaurant )
    ...
    <div class="row p-3">
        <div class="col-md-12 text-right">
            @if($availability == 1)
                <button class="btn btn-primary" type="button" onclick="changeToUnAvailableAll()">
                    Unavailable All
                </button>
            @else
                <button class="btn btn-primary" type="button" onclick="changeToAvailableAll()">
                    Available All
                </button>
            @endif
        </div>
    </div>
    <div class="col-md-12 mt-3" id="found_available">
        <table class="table table-hover sortable" id="itemTable">
            ...
            <tr>
                ...
                <th>Image</th>
                <th>Item Name</th>
                <th>Status(Click to change the status)</th>
            </tr>
            </thead>
            <tbody id="all-side-menus-list" class="">
            @foreach($items as $key=>$value)
                <tr>
                    <td>{{$value->item_id}}</td>
                    @if ($value->item_thumbnail)
                        <td><img class="card-img-left" src="{{url('/')}}/public/storage/{{$value->item_thumbnail}}"
                                 alt="" style="height: 60px;width: 60px;"></td>
                    @else
                        <td><img class="card-img-left" src="{{url('/')}}/public/assets/images/menu-default.png" alt=""
                                 style="height: 60px;width: 60px;"></td>
                    @endif
                    <td>{{$value->item_name}}</td>
                    <td>@if($value->is_in_stock == 1)
                            <div class="no-checkedselector">
                                <div class="toggle-container">
                                    <input id="toggle-on-{{$value->item_id}}" data-item="{{$value->item_id}}" class="toggle toggle-left checked" name="toggle-{{$value->item_id}}" type="radio" checked>
                                    <label for="toggle-on-{{$value->item_id}}" class="btn">In-Stock</label>
                                    <input id="toggle-off-{{$value->item_id}}" data-item="{{$value->item_id}}" class="toggle toggle-right" name="toggle-{{$value->item_id}}" type="radio">
                                    <label for="toggle-off-{{$value->item_id}}" class="btn">No Stock</label>
                                </div>
                            </div>
                        @else
                            <div class="no-checkedselector">
                                <div class="toggle-container">
                                    <input id="toggle-on-{{$value->item_id}}" data-item="{{$value->item_id}}" class="toggle toggle-left" name="toggle-{{$value->item_id}}" type="radio">
                                    <label for="toggle-on-{{$value->item_id}}" class="btn">In-Stock</label>
                                    <input id="toggle-off-{{$value->item_id}}" data-item="{{$value->item_id}}" class="toggle toggle-right checked" name="toggle-{{$value->item_id}}" type="radio"  checked>
                                    <label for="toggle-off-{{$value->item_id}}" class="btn">No Stock</label>
                                </div>
                            </div>
                        @endif
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-12" id="showCategory">
                                    <div class="form-group">
                                        ...
                                        <select id="category" class="form-control selectpicker" multiple
                                ...
                                <div class="col-md-12">
                                    <div class="form-group">
                                        ...
                                        <textarea style="resize: none" class="form-control" placeholder="Description"
                                                  ...
                                                  onkeydown="limitText(this.form.message,this.form.countdown,60);"
                                                  onkeyup='limitText(this.form.message,this.form.countdown,60);'></textarea>
                                        <input readonly type="text" name="countdown" size="1"
                                               class="text-right remainingCharacter" value="60 "><span
                                            class="totalRemainingCharacter">/ 60</span>
                                    </div>
                                </div>
                                <div class="col-md-12">
                                    <div class="form-group">
                            ...
                            <div class="col-md-12">
                                <div class="form-group">
                                    ...
                                    <textarea style="resize: none" class="form-control" placeholder="Description"
                                              ...
                                              onkeydown="limitEditText(this.form.desc,this.form.countdownDesc,60);"
                                              onkeyup='limitEditText(this.form.desc,this.form.countdownDesc,60);'></textarea>
                                    <input readonly type="text" name="countdownDesc" size="1"
                                           class="text-right remainingCharacter" value="60 "><span
                                        class="totalRemainingCharacter">/ 60</span>
                                </div>
                            </div>
...
@section('bottom-js')
    <script>
        $(document).on('change', '.no-checkedselector input[type="radio"].toggle', function () {
            if (this.checked) {
        ...
        function searchMenu(flag) {
            ...
            axios.post(baseUrl + 'search-item-availability', data).then(function (response) {
                $('#searching').hide();
                if (response.data.response.length) {
                    ...
                    for (i = 0; i < response.data.response.length; i++) {
                        ...
                        if(obj.is_in_stock == 1){
                            var stock = 'Available';
                            //var button = '<a title="Change To Unavailable" class="action-links" onclick="changeToUnAvailable(' + obj.item_id + ')"><i class="fa fa-times" aria-hidden="true"></i></a> <input type="checkbox" class="toggle-btn" checked data-toggle="toggle">';
                            var button = `<div class="no-checkedselector">
                                <div class="toggle-container">
                                    <input id="toggle-on-`+obj.item_id+`" data-item="`+obj.item_id+`" class="toggle toggle-left checked" name="toggle-`+obj.item_id+`" type="radio" checked>
                                        <label for="toggle-on-`+obj.item_id+`" class="btn">In-Stock</label>
                                        <input id="toggle-off-`+obj.item_id+`" data-item="`+obj.item_id+`" class="toggle toggle-right" name="toggle-{{$value->item_id}}" type="radio">
                                            <label for="toggle-off-`+obj.item_id+`" class="btn">No Stock</label>
                                </div>
                            </div>`;
                        } else{
                            var stock = 'Not Available';
                            //var button = '<a title="Change To Available" class="action-links" onclick="changeToAvailable(' + obj.item_id + ')"><i class="fa fa-check" aria-hidden="true"></i></a> <input type="checkbox" class="toggle-btn" checked data-toggle="toggle">';
                            var button = `<div class="no-checkedselector">
                                <div class="toggle-container">
                                    <input id="toggle-on-`+obj.item_id+`" data-item="`+obj.item_id+`" class="toggle toggle-left" name="toggle-`+obj.item_id+`" type="radio">
                                        <label for="toggle-on-`+obj.item_id+`" class="btn">In-Stock</label>
                                        <input id="toggle-off-`+obj.item_id+`" data-item="`+obj.item_id+`" class="toggle toggle-right checked" name="toggle-`+obj.item_id+`" type="radio"  checked>
                                            <label for="toggle-off-`+obj.item_id+`" class="btn">No Stock</label>
                                </div>
                                </div>`;
                        }
                        ...
                        var rowHtml = `<tr>`+
                            `<td>`+ obj.item_id +`</td>`+
                            `<td><img class="card-img-left" src="`+obj.item_thumbnail+`" alt="" style="height: 60px;width: 60px;"></td>` +
                            `<td>`+ obj.item_name +`</td>`+
                            `<td>`+button+`</td>`+
                            `</tr>`;
                        $('#all-side-menus-list').append(rowHtml);
        ...
        $(document).on('change','.button-switch .switch',function() {
            var item = $(this).attr('data-item');
            if($(this).prop('checked') == true){
                changeToAvailable(item);
            } else{
                changeToUnAvailable(item);
            }
        });

        $(document).on('change','.toggle-container .toggle',function(){
            var item = $(this).attr('data-item');
        ...
        function changeToUnAvailable(itemId){
            axios.get(baseUrl + 'change-item-to-unavailable/' + itemId).then(function (response) {
                toastr.success(response.data.message, "Success", {
                    timeOut: "3000",
                    positionClass: "toast-bottom-right"
                });
                $('#item-av-'+itemId).text('Out Of Stock');
            });
        }

        function changeToAvailable(itemId){
            axios.get(baseUrl + 'change-item-to-available/' + itemId).then(function (response) {
                toastr.success(response.data.message, "Success", {
                    timeOut: "3000",
                    positionClass: "toast-bottom-right"
                });
                $('#item-av-'+itemId).text('In Stock');
            });
        }

        function changeToUnAvailableAll(){
            axios.get(baseUrl + 'all-items-unavailable/').then(function (response) {
                toastr.success(response.data.message, "Success", {
                    timeOut: "3000",
                    positionClass: "toast-bottom-right"
                });
                $('.item-av').text('Out Of Stock');
                window.location.reload();
            });
        }

        function changeToAvailableAll() {
            axios.get(baseUrl + 'all-items-available/').then(function (response) {
                toastr.success(response.data.message, "Success", {
                    timeOut: "3000",
                    positionClass: "toast-bottom-right"
                });
                $('.item-av').text('In Stock');
                window.location.reload();
            });
        }
