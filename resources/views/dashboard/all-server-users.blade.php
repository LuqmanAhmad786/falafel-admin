...
@section('main-content')
    ...
    <ul class="nav nav-tabs" id="myTab" role="tablist">
        ...
        <li class="nav-item">
            <a class="nav-link active" href="{{route('server-users')}}">Server App User(s)</a>
        </li>
    </ul>
    <div class="row">
        <div class="col-md-6">
            <h1 class="heading-white mb-4">Server App User(s) [{{$count}}]</h1>
        </div>
        <div class="col-md-6 mt-1">
            <button class="btn btn-primary float-right" data-target="#addNewManager" data-toggle="modal">Add Server User
            </button>
        </div>
        <div class="col-md-12">
            <table class="table table-hover sortable" id="managerTable">
                ...
                @foreach($managers as $key=>$value)
                    <tr>
                        <td>{{$key+1}}</td>
                        <td class="first-row">{{$value->first_name}} {{$value->last_name}}</td>
                        <td>{{$value->email}}</td>
                        <td>@if($value->assigned_restaurant) {{$value->name ? $value->name : '-'}} @else - @endif</td>
                        <td>
                            @if($value->is_account_deleted == 0)
                                <label class="badge badge-success">Active</label>
                            @endif
                            @if($value->is_account_deleted == 1)
                                    <label class="badge badge-danger">In-Active</label>
                            @endif
                        </td>
                        <td>
                            @if($value->is_account_deleted == 0)
                                <a class="action-links ml-1" onclick="changeStatus('{{$value->id}}',1)">Deactivate</a>
                            @endif
                            @if($value->is_account_deleted == 1)
                                <a class="action-links ml-1" onclick="changeStatus('{{$value->id}}',0)">Activate</a>
                            @endif
                        </td>
                    </tr>
                @endforeach
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Add Server User</h5>
...
@section('bottom-js')
    <script>
        ...
        function addNewManager() {
            ...
            axios.post(baseUrl + 'add-new-server-user', data).then(function (response) {
        ...
        function changeStatus(userId, status){
            ...
            if(c){
                ...
                axios.post(baseUrl + 'update-server-user-status', data).then(function (response) {
                    toastr.success(response.data.message, "Success", {
                        timeOut: "3000",
                        positionClass: "toast-bottom-right"
                    });
