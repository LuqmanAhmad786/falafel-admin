...
@section('main-content')
    <ul class="nav nav-tabs" role="tablist">
        <li class="nav-item col-md-2 text-center">
            <a class="nav-link" href="{{route('menu-breakfast')}}">
                Breakfast
            </a>
        </li>
        <li class="nav-item col-md-2 text-center">
        ...
        <div class="tab-pane fade show active">
            <div class="row">
                <div class="col-md-3">
                    <div class="card mb-4">
                        <div class="card-body">
                            <div class="text-center text-20 mt-5" id="add_new_action" onclick="clickAddMenu()">
                                <a href="javascript:void(0)">
                                    <i class="i-Add"></i>
                                    <div>Add New Item</div>
                                </a>
                            </div>
                            {{-- <div class="row" id="add_new_menu_form" style="display: none;">
                                 ...
                                 <div class="col-md-12 text-right">
                                     <button type="button" class="btn btn-secondary m-1" onclick="hideAddMenu()">Cancel
                                     </button>
                            ...
                            <div class="card-body" id="view_break_fast_{{$item->id}}">
                                ...
                                <div class="row mt-1 mb-2 pl-3 text-primary">
                                    {{-- @if(sizeof($item->subMenu))
                                         @foreach($item->subMenu as $key => $cat)
                                             <label class="badge badge-primary ml-1">{{$cat->name}}{{$key !== (sizeof($item->subMenu) - 1) ? ',' : ''}}</label>
                                         @endforeach
                                     @endif--}}
                                </div>
                                @if($item->thumbnail)
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-12 text-center mt-2 ml-2">
                                        <button title="Edit" class="btn btn-primary" onclick="doEdit({{$item}})"><i
                                                class="i-Pen-2"></i>
                                        </button>
...
@section('bottom-js')
    <script>
        ...
        function doEdit(item) {
            ...
            isAction = 'Edit';
            if (item.sub_menu.category_id) {
                $('#editCategory').val(item.sub_menu.category_id);
            }
        }

        function cancelEdit(itemId) {
            const viewBreakfast = '#view_break_fast_' + itemId;
            const editBreakfast = '#edit_break_fast_' + itemId;
            $(viewBreakfast).show();
            $(editBreakfast).hide();
        }
            ...
            Swal.fire({
                ...
                if (result.value) {
                    axios.get(baseUrl + 'delete-items/' + itemId + '/' + table).then(function (response) {
                        toastr.success('Lunch Menu Deleted Successfully.', "Success", {
                            timeOut: "3000",
                            positionClass: "toast-bottom-right"
                        });
                        setTimeout(function () {
        ...
        function getCategories(menuId) {
            $('#editCategory').html('');
            axios.get(baseUrl + 'get-categories/' + menuId + '/main').then(function (response) {
                allCategories = response.data.response;
                for (let i = 0; i < allCategories.length; i++) {
                    if (allCategories[i].checked) {
                        $('#editCategory').append('<option selected value="' + allCategories[i].id + '">' + allCategories[i].name + '</option>');
                    } else {
                        $('#editCategory').append('<option value="' + allCategories[i].id + '">' + allCategories[i].name + '</option>');
                    }
                }
            });
        }
